...
public class ConceptValidatorTest extends BaseContextSensitiveTest {
	 ...
	 **/
	@Test
	@Verifies(value = "should pass if  none of  ConceptDescription are Null", method = "validate(Object,Errors)")
	public void validate_shouldPassIfNoneOfDescriptionAreNull() throws Exception {
		Concept concept = new Concept();
		List<Locale> locales = Context.getAdministrationService().getAllowedLocales();
		for (Locale loc : locales) {
			concept.addDescription(new ConceptDescription("Not Empty Description", loc));
		}
		Errors errors = new BindException(concept, "concept");
		new ConceptValidator().validate(concept, errors);
